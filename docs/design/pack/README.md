...
```text
BM_QuestHub_Media/
├── FOR_DESIGNER/          ← содержимое архива (PDF, PSD, CSV, previews, …)
│   ├── 01_Инструкция_дизайнеру.pdf
│   ├── templates-psd/
│   └── …
├── quests/{slug}/         ← рабочие исходники дизайнера
└── …
```

`inbox-README.txt` из пакета — для корня `BM_QuestHub_Media` (можно скопировать вручную один раз).
