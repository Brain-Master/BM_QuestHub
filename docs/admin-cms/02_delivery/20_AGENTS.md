...
- Package manager: `npm`.
- Node.js baseline: major `22`; точную версию сверять с CI/tooling, не обновлять самостоятельно.
...
- API contract target: `docs/admin-cms/contracts/admin-api.openapi.yaml`; до его создания endpoint changes требуют отдельного Task Packet.
- Generated/public artifacts по умолчанию защищены:
  - `apps/web/data/**`;
  - generated snapshot/detail files;
  - build outputs;
  - generated media variants.
- Секретные пути всегда запрещены к чтению/выводу/коммиту без специального разрешения:
  - `secret/**`;
  - `*.env` и `*.env.*`, кроме явно публичных `.env.example`;
  - credential files;
  - private keys, cookies, PAT, service-account JSON.
- Workflow и production deployment paths защищены по умолчанию:
  - `.github/workflows/**`;
  - deployment scripts;
  - S3 migration scripts;
  - production environment configuration.
- Secret scan command должен быть задан Task Packet или существующей стабильной репозиторной командой. Если команды нет, commit запрещён до решения владельца.
...
5. Документы `docs/admin-cms/`.
6. Package-local документация.
7. Существующие соглашения.
8. Defaults инструментов.
...
Для material task создать/обновить ExecPlan по `25_EXECPLAN_TEMPLATE.md`.
...
По мере появления stable scripts использовать:
...
<SECRET_SCAN_COMMAND>
<ALL_REQUIRED_STAGED_VALIDATION>
```
...
Authorized branch: <EXACT_BRANCH>
Expected HEAD: <BASE_SHA>
Task: <TASK_ID_AND_TITLE>

Objective: <ONE_ATOMIC_OUTCOME>
Non-goals: ...
