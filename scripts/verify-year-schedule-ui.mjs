...
try {
 ...
 assert.equal(await cards.locator('a[href*="/card/"]').getAttribute('href'),'https://www.mos.ru/pgu2/activity/card/965306');
 ...
 await page.getByRole('combobox',{name:'Адрес',exact:true}).selectOption('school-1212-golubinskaya-21k3');
 ...
 for(const path of ['/year-courses/','/year-courses/shmi/']) {
  await page.goto(base+path,{waitUntil:'networkidle'});
  await page.locator('.ys-entry a').waitFor();
  const entry=new URL(await page.locator('.ys-entry a').getAttribute('href'),base);
  assert.equal(entry.pathname,'/agenda/');assert.equal(entry.searchParams.get('view'),'wizard');
  if(path.includes('/shmi/'))assert.equal(entry.searchParams.get('programme'),'shmi');
  await page.locator('.ys-entry a').click();
  await page.getByRole('heading',{name:'Где будем заниматься?',exact:true}).waitFor();
 }
 await page.goto(base+'/agenda/',{waitUntil:'networkidle'});
 assert.equal(await page.getByTestId('finder-count').innerText(),'51');
 ...
 checks.push('Overview / SHMI enter the real wizard; full agenda has 51 groups; old table URL retained; no annual workspace writes or runtime errors');
