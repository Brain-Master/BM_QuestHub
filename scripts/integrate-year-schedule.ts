...
const generatedAt = `${source.asOf}T00:00:00.000Z`;
...
const venues = annualLocations.filter(l => !l.existing).map(l => {
  ...
  return venueSchema.parse({ slug: l.slug, name: row.school, type: "school", schoolScopeSlug: l.schoolScopeSlug,
    address: l.address, city: "moscow", latitude: l.latitude, longitude: l.longitude, listedOnSites: true,
...
const annualOffers = source.groups.map(g => {
  ...
  const name = annualProgrammeName(g.title);
  return venueOfferSchema.parse({ id: `year:${g.id}`, venueSlug: location.slug, shiftLabel: name.short,
    ...
    annual: { sourceTitle: g.title, asOf: source.asOf, sourceSha256: source.sourceSha256, listingId: g.listingId, groupCode: g.groupCode,
    ...
    scheduleCard: { displayTitle: name.short, teacherName: g.teacher, description: `Данные mos.ru на ${source.asOf}. Условия и наличие мест проверьте перед записью.`,
