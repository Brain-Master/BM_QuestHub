...
/** When true, tariffs render in a full-width footer (reference: Footer Placement Pattern). */
export function placeVariantsInFooter(variantsCount: number): boolean {
  return variantsCount > 2;
}

type TariffSwapZoneProps = {
  item: ScheduleBoardItem;
  schoolSlug?: string;
  bookingSchoolSlug?: string;
  expanded: boolean;
...
function TariffSwapZone({
  ...
  const bookingSlug = bookingSchoolSlug ?? schoolSlug;

  return (
    <div className="relative flex min-h-[140px] w-full flex-col">
      <div
        className={cn(
      ...
      >
        ...
        <div data-testid="variants-container">
          <ScheduleTariffList item={item} schoolSlug={bookingSlug} compact />
        </div>
      </div>
...
export function ScheduleBoardCardCompact({
  ...
  const useFooterForVariants = placeVariantsInFooter(variantsCount);
  ...
  return (
    <article
      ...
      className={cn(
        "group/schedule-card isolate overflow-hidden rounded-2xl border border-[color:var(--schedule-card-border)] bg-[color:var(--schedule-card-bg)] shadow-[var(--schedule-card-shadow)] backdrop-blur-md transition",
        !useFooterForVariants && "lg:flex lg:h-[25rem]",
        useFooterForVariants && "lg:flex lg:flex-col",
    ...
    >
      <div
        className={cn(
          "min-w-0",
          useFooterForVariants ? "lg:flex lg:min-h-0 lg:flex-1" : "lg:flex lg:flex-1",
        )}
      >
        ...
        <div className="relative z-20 flex min-w-0 flex-1 flex-col gap-2 overflow-hidden p-3.5">
          ...
          <div className="min-w-0">
            ...
            >
              <ScheduleCardTitle
                item={item}
                headingClassName="font-heading text-xl font-semibold leading-tight"
              />
              <ArrowUpRight
          ...
          {useFooterForVariants ? (
            <div className="mt-1 min-h-0 flex-1 overflow-hidden">
              <p className="line-clamp-3 text-muted-foreground text-sm leading-relaxed min-[1180px]:line-clamp-4">
                {item.description}
              </p>
            </div>
          ) : (
      ...
      {useFooterForVariants ? (
        <div
          data-testid="schedule-card-tariff-footer"
          className="w-full border-white/10 border-t bg-[color:var(--schedule-card-bg)] px-3.5 py-3.5 lg:px-5 lg:py-4"
        >
          <TariffSwapZone
            item={item}
