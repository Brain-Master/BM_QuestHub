...
test('four historical removals and eight replacements cannot pollute ordinary facets',()=>{
 const annual=quests.flatMap(q=>q.offers).filter(o=>o.annual);
 const removed=annual.filter(isRetiredAnnualGroup);assert.equal(annual.length,74);assert.equal(removed.length,12);
 assert.deepEqual([...new Set(removed.map(o=>o.annual!.listingId))].sort(),['2548560','2548561','2549843','2549844','2549845']);
 ...
 const view=projectAnnualWorkspace(old,venues);assert.equal(view.groups.length,62);assert.ok(view.groups.every(g=>!isRetiredAnnualGroup(g)));
