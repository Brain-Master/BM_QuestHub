...
test('exact four historical flags; old snapshots cannot offer registration or pollute ordinary facets',()=>{
 const annual=quests.flatMap(q=>q.offers).filter(o=>o.annual);
 const removed=annual.filter(isRetiredAnnualGroup);assert.equal(annual.length,74);assert.equal(removed.length,4);
 assert.deepEqual(removed.map(o=>o.annual!.listingId).sort(),['2548561','2549843','2549844','2549845']);
 for(const o of removed)assert.equal(o.scheduleCard?.isArchived,true);
 const old=quests.map(q=>({...q,offers:q.offers.map(o=>isRetiredAnnualGroup(o)?{...o,id:`old:${o.id}`,scheduleCard:{...o.scheduleCard,isArchived:false}}:o)}));
 ...
 for(const item of items.filter(i=>isRetiredAnnualGroup(i.offer))){
  ...
  assert.equal(finderItemMatches(item,{...state,offer:item.offer.id}),true);
 ...
 const view=projectAnnualWorkspace(old,venues);assert.equal(view.groups.length,70);assert.ok(view.groups.every(g=>!isRetiredAnnualGroup(g)));
 const school875=annual.find(o=>o.annual?.listingId==='2463216');assert.ok(school875);assert.equal(isRetiredAnnualGroup(school875),false);
 assert.ok(view.groups.some(g=>g.listingId==='2463216'));
});
test('availability cannot resurrect/modify a retired old-hot record; unrelated closed/full remain active',()=>{
 ...
 const old={...retired,scheduleCard:{...retired.scheduleCard,isArchived:false}};
 ...
 const kept=q.offers.find(o=>o.annual?.listingId==='2463216')!;
